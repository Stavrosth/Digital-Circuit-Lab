module uart_transmitter(reset, clk, Tx_DATA, baud_select, Tx_WR, Tx_EN, TxD, Tx_BUSY);//// clk_out_use, current_state_use, next_state_use, select);
    ...
    send_bits transmit(.clk_out(clk_out_slow), .reset(reset), .Tx_WR(Tx_WR), .Tx_EN(Tx_EN), .Tx_DATA(Tx_DATA), .Tx_BUSY(Tx_BUSY), .TxD(TxD));//, .current_state(current_state_use), .next_state(next_state_use), .select(select));
endmodule