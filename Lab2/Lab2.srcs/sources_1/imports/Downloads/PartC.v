module uart_receiver(reset, clk, Rx_DATA, baud_select, Rx_EN, RxD, Rx_FERROR, Rx_PERROR, Rx_VALID);//, current_state, next_state, counter_begin, temp, select_begin, starter, clk_out_slow_use, clk_out_use);
    ...
    receive_bits receive(.clk_out(clk_out), .clk_out_slow(clk_out_slow), .reset(reset), .Rx_EN(Rx_EN), .RxD(RxD), .Rx_DATA(Rx_DATA), .Rx_FERROR(Rx_FERROR), .Rx_PERROR(Rx_PERROR), .Rx_VALID(Rx_VALID));//, current_state, next_state, counter_begin, temp, select_begin, starter);
endmodule