module receive_bits(clk_out, clk_out_slow, reset, Rx_EN, RxD, Rx_DATA, Rx_FERROR, Rx_PERROR, Rx_VALID);//, current_state, next_state, counter, temp, select, starter);
    ...
    always @(current_state or select) begin
        ...
        case (current_state)
            4'd0: begin
                ...
                if (select == 1'b1) //allows the fsm to move to the next state or not
                    next_state = 4'd1;
                else 
                    next_state = 4'd0;
            end
    ...
    always @(posedge clk_out_slow) begin
        if (reset == 1'b1 || Rx_EN == 1'b0 || select == 1'b0) begin
            current_state <= 4'd0;
            temp <= 8'b11111111;
        end else
    ...
    always @(posedge clk_out) begin
        if ( reset == 1'b1) begin //resets all the necessary values
            ...
            counter <= counter + 1'b1; //enables the counter to increase for 24 times after start bit is detected
