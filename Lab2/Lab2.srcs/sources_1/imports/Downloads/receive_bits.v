module receive_bits(clk_out, reset, Rx_EN, RxD, Rx_DATA, Rx_FERROR, Rx_PERROR, Rx_VALID, current_state, next_state, counter, temp, select, starter);
    ...
    always @(current_state or select) begin
        ...
        case (current_state)
            ...
            4'd1: begin
               //variable so it can sesnd it to the system in parallel
    ...
    always @(posedge clk_out) begin
        if ( reset == 1'b1 || ( next_state == 4'b0 && select == 2'b01)) begin //resets all the necessary values
            counter <= 4'b0;
            second_counter <= 3'b0;
            select <= 2'b00;
            starter <= 1'b0;
            ...
            if ( RxD == 1'b0 && starter == 1'b0) begin    
                ...
                if ( second_counter  == 3'b111) begin
